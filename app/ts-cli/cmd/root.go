...
func init() {
	rootCmd.Flags().StringVar(&options.Host, "host", DEFAULT_HOST, "openGemini host to connect to.")
	rootCmd.Flags().IntVar(&options.Port, "port", DEFAULT_PORT, "openGemini port to connect to.")
	...
	rootCmd.Flags().StringVar(&options.Precision, "precision", "ns", "Precision specifies the format of the timestamp:  rfc3339,h,m,s,ms,u or ns.")
}
